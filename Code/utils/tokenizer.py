...
class CustomizedTokenizer(Tokenizer):
    ...
    def __init__(self, preserve_case=False):
        super(CustomizedTokenizer,self).__init__(preserve_case)

    def tokenize(self, s):
        """
        Override the Tokenizer 'tokenize'
        method to ensure it better handles
        incorrect types
        """

        if isinstance(s, str):
            return super(CustomizedTokenizer,self).tokenize(s)
        else:
...
class NegationSuffixAdder():
    ...
    PUNCT_RE = re.compile("^[.:;!?]$")
    #regex to match shift
    PUNCT_TRANSITION_WORDS=re.compile("""(?x)(?:
    ^(?:but|while|although|though|albeit|unlike|nevertheless|nonetheless
     )$
    )
    """)
    ...
    def add_negation_suffixes(self, tokens):
        ...
        for token in tokens:
            ...
            if self.PUNCT_RE.match(token):
                append_neg = False
            if append_neg == True and self.PUNCT_TRANSITION_WORDS.match(token):
                append_neg=False
